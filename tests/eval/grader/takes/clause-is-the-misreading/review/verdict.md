...
0 disagreement · 0 cannot tell · 0 no reading · 0 not demonstrated · 2 agreement · 1 of them before the first claimed beat
...
- **Reader:** seen, `beat-04.png` (beat 4), confidence medium — The search box above the queue holds the typed text 'export' with the caret still in it, the list has narrowed from 7 tickets to 1 and the header count reads (1), and the only control beside the box is a clear '×' — no submit button exists on screen — though still frames cannot show per-keystroke narrowing, so an Enter-press cannot be fully ruled out.
...
### AC-2 — Search matches the ticket title — typing a word from the title narrows the queue to that ticket.

- **agreement, before the first claimed beat** — The reader located this clause at beat 4 (`beat-04.png`), 3 beat(s) before the claimed span [7, 16] — it was already visible before anything claimed it. The reader was asked for the *first* frame showing the clause, so this is a lower bound and contradicts nothing the storyboard said. It does not prove the clause was still visible on the claimed beats.
  *Derived by:* demo-grade, comparing the reader's answer with the beats the storyboard claimed — the rule, including the span it uses when both derivations locate the clause, is stated in this document.
- **Reader:** seen, `beat-04.png` (beat 4), confidence high — Typing 'export', a word from the title of TQ-102 'Export to CSV times out on large accounts', narrows the queue to exactly that ticket — and the later 'dashboard' pass (beat-10, with the matching title outlined in beat-14) corroborates that the match is in the title.
  *Derived by:* a reader given the clause text and the frame filenames, and neither the storyboard, the ac tags, the coverage table nor the diff.
