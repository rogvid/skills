...
0 disagreement · 0 cannot tell · 0 no reading · 0 not demonstrated · 2 agreement
...
### AC-1 — Choosing a status above the queue narrows the list to the tickets with that status.

- **agreement, inside the claimed span** — The reader located this clause at beat 4 (`beat-04.png`), inside the claimed span [2, 11].
...
- **Reader:** seen, `beat-04.png` (beat 4), confidence high — The Open pill above the list is now the selected (dark) one and the list has shrunk from seven rows to exactly the four rows badged 'open' (TQ-101, TQ-102, TQ-104, TQ-106), with the heading count changing from 7 to 4.
...
- **Storyboard:** claimed beats [2, 6, 7, 11], span [2, 11]
...
### AC-2 — Choosing All restores the whole queue, and the heading above it names the queue being listed.

- **agreement, inside the claimed span** — The reader located this clause at beat 14 (`beat-14.png`), inside the claimed span [12, 17].
  *Derived by:* demo-grade, comparing the reader's answer with the beats the storyboard claimed — the rule, including the span it uses when both derivations locate the clause, is stated in this document.
- **Reader:** seen, `beat-14.png` (beat 14), confidence high — The All pill is selected and all seven tickets are back in the list, with the heading above it reading 'Support queue (7)' and visibly highlighted as the queue being listed.
  *Derived by:* a reader given the clause text and the frame filenames, and neither the storyboard, the ac tags, the coverage table nor the diff.
- **Storyboard:** claimed beats [12, 17], span [12, 17]
  *Derived by:* coverage.claimed in timeline.json — the ac= tags the storyboard author typed, which are evidence of intent and nothing else.
