...
1 disagreement · 0 cannot tell · 0 no reading · 0 not demonstrated · 1 agreement · 1 of them before the first claimed beat
...
### AC-2 — A search box above the queue narrows the list as you type, with no button to press.

- **disagreement** — The storyboard claimed this clause on beats [11, 14] and the reader could not find it in any frame.
...
- **Reader:** not seen, confidence high — A search field sits above the list and is highlighted in beats 12-14, but it never receives a character - the placeholder stays in it through the final frame and the list remains all seven rows, so narrowing-as-you-type is never shown, only asserted by the caption.
...
- **Storyboard:** claimed beats [11, 14], span [11, 14]
...
### AC-1 — The queue lists every ticket with its id, its title and the person who raised it.

- **agreement, before the first claimed beat** — The reader located this clause at beat 0 (`beat-00.png`), 2 beat(s) before the claimed span [2, 9] — it was already visible before anything claimed it. The reader was asked for the *first* frame showing the clause, so this is a lower bound and contradicts nothing the storyboard said. It does not prove the clause was still visible on the claimed beats.
  *Derived by:* demo-grade, comparing the reader's answer with the beats the storyboard claimed — the rule, including the span it uses when both derivations locate the clause, is stated in this document.
- **Reader:** seen, `beat-00.png` (beat 0), confidence high — The queue list is on screen with seven rows, each showing an id (TQ-101 to TQ-107), a bold title, and a requester address such as dana.olsen@northwind.example.
  *Derived by:* a reader given the clause text and the frame filenames, and neither the storyboard, the ac tags, the coverage table nor the diff.
- **Storyboard:** claimed beats [2, 4, 5, 9], span [2, 9]
  *Derived by:* coverage.claimed in timeline.json — the ac= tags the storyboard author typed, which are evidence of intent and nothing else.
