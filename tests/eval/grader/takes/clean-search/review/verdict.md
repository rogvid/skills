...
- **agreement, inside the claimed span** — The reader located this clause at beat 4 (`beat-04.png`), inside the claimed span [2, 12].
...
- **Reader:** seen, `beat-04.png` (beat 4), confidence high — The search field above the ticket list contains the typed text 'invoice' and the queue has narrowed from seven rows to the single matching TQ-101 with the header count changed to (1), with no submit button anywhere in the control.
...
- **Storyboard:** claimed beats [2, 6, 7, 12], span [2, 12]
...
### AC-2 — A search matching nothing shows a 'No tickets match this filter.' line instead of a blank queue.

- **agreement, inside the claimed span** — The reader located this clause at beat 16 (`beat-16.png`), inside the claimed span [13, 18].
  *Derived by:* demo-grade, comparing the reader's answer with the beats the storyboard claimed — the rule, including the span it uses when both derivations locate the clause, is stated in this document.
- **Reader:** seen, `beat-16.png` (beat 16), confidence high — With 'refund' typed in the search field the header reads (0) and the list area shows the line 'No tickets match this filter.' rather than an empty region.
  *Derived by:* a reader given the clause text and the frame filenames, and neither the storyboard, the ac tags, the coverage table nor the diff.
- **Storyboard:** claimed beats [13, 18], span [13, 18]
  *Derived by:* coverage.claimed in timeline.json — the ac= tags the storyboard author typed, which are evidence of intent and nothing else.
